...
app.get('/homepage-data', async (req, res) => {
    ...
    try {
        ...
        const memberData = await Member.findOne({ member_name: "Mwebe Blaise" });
        const constants = await Constants.findOne();
        const club = await Member.find({});
        const memberDeposits = await Deposit.find({ depositor_name: "Mwebe Blaise" });
        const debts = await Loans.find({ borrower_name: "Mwebe Blaise", loan_status: "Ongoing" });
        const debtHistory = await Loans.find({ borrower_name: "Mwebe Blaise"});
        const earnings = await Earnings.find({ beneficiary_name: "Mwebe Blaise" });
        const points = await PointsSale.find({ name: "Mwebe Blaise", type: "Sell"});
        const units = await InvestmentUnits.find({ name: "Mwebe Blaise" });
        ...
        const clubWorth = processArray(club, (c) => c.reduce((total, member) => total + member.investment_amount, 0));
        ...
        const memberYears = memberData ? Math.round((getDaysDifference(memberData.date_of_membership) / 365) * 10) / 10 : 'No Data Available';
        
        const one_point_value = ((constants.max_lending_rate - constants.min_lending_rate) * 2 * memberData.investment_amount * 25) / (500 * 100 * 12);
        const pointsWorth = Math.round(one_point_value * memberData.points);
        const member_risk = Math.round(memberData.investment_amount * 100 /clubWorth);// + '%';
        const totalDebt = debts.reduce((total, loan) => total + loan.principal_left + loan.interest_amount, 0);
        const possiblePoints = (memberData.points  / 25);
        ...
        const maxLimit = memberData.investment_amount * 20 <= clubWorth ? (memberData.investment_amount * 5 - totalDebt): ((0.25 * clubWorth) - totalDebt);
        ...
        if (depositsArray !== 'No Data Available') {
            sortedDepositYears.forEach(([year, record]) => {
                ...
                memberDepositsRecords.push({
                    ...
                    avgMonthyDeposit: year !== currentYear ? record.deposit_amount / 12 : record.deposit_amount / (new Date().getMonth() + 1),
                    values
                });
            });
        }
            ...
            sortedEarningsYears.forEach(([year, record]) => {
                units.forEach(entry => {
                    if (entry.year === year) {
                        ...
                        actualInvestmentTotal += actualInvestment;
                        let yearObject = {
                            year: year,
                            total: record.earnings_amount,
                            ...
                            values: processArray(earningsArray.recordsByYear[year], (earningsRecords) =>
                                ...
                                earningsRecord.earnings_amount,
            ...
            Object.entries(debtRecords.recordsByYear).forEach(([year, records]) => {
                records.forEach(record => {
                    ...
                    let yearObject = {
                        ...
                        amountLeft: record.principal_left + record.interest_amount,
        ...
        if (clubEarningsArray !== 'No Data Available') {
            sortedClubEarningsYears.forEach(([year1, record]) => {
                Object.entries(clubUnits.yearsSums).forEach(([year2, record2]) => {
                    if (year2 === year1) {
                        ...
                        const year = year1;
                        const total = record.earnings_amount;
        ...
        if (pointsArray !== 'No Data Available') {
            sortedPoints.forEach(([year, record]) => {
                let values = pointsArray.recordsByYear[year].map(pointsRecord => [
                    ...
                    pointsRecord.points_involved,
                    pointsRecord.reason
                ]);

                let yearObject = {
                    year: year,
                    total: record.points_involved,
                    values: values
                };
        ...
        return res.json({
                memberDashboard: {
                    summary: {
                        memberDeposits: {
                            yourWorth: memberData.investment_amount,
                        },
                        payments: {
                            avgYearlyReturn: averageEarnings,
                        },
                        loans: {
                    ...
                    home: {
                        ...
                        yourWorth: memberData.investment_amount,
                        risk: member_risk,
                        thisYearSavings: depositsArray.yearsSums["2023"] ? depositsArray.yearsSums["2023"].deposit_amount : 'No Data',
                        yourDebt: totalDebt,
